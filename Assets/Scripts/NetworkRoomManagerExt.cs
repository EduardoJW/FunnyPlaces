...
namespace Mirror
{
    ...
    /// <para>NetworkRoomManagerExt is derived from NetworkManager, and so it implements many of the virtual functions provided by the NetworkManager class. To avoid accidentally replacing functionality of the NetworkRoomManagerExt, there are new virtual functions on the NetworkRoomManagerExt that begin with "OnRoom". These should be used on classes derived from NetworkRoomManagerExt instead of the virtual functions on NetworkManager.</para>
    /// <para>The OnRoom*() functions have empty implementations on the NetworkRoomManagerExt base class, so the base class functions do not have to be called.</para>
    /// </remarks>
    [AddComponentMenu("Network/NetworkRoomManagerExt")]
    [HelpURL("https://mirror-networking.com/docs/Components/NetworkRoomManagerExt.html")]
    public class NetworkRoomManagerExt : NetworkManager
    {
        static readonly ILogger logger = LogFactory.GetLogger(typeof(NetworkRoomManagerExt));
        ...
        {
            logger.Log("NetworkRoomManagerExt OnServerReady");
        ...
        public override void OnServerAddPlayer(NetworkConnection conn)
        {
            ...
            if (IsSceneActive(RoomScene))
            {
                ...
                if (logger.LogEnabled()) logger.LogFormat(LogType.Log, "NetworkRoomManagerExt.OnServerAddPlayer playerPrefab:{0}", roomPlayerPrefab.name);
        ...
        public override void OnStartServer()
        {
            ...
            {
                logger.LogError("NetworkRoomManagerExt RoomScene is empty. Set the RoomScene in the inspector for the NetworkRoomMangaer");
                return;
            }

            if (string.IsNullOrEmpty(GameplayScene))
            {
                logger.LogError("NetworkRoomManagerExt PlayScene is empty. Set the PlayScene in the inspector for the NetworkRoomMangaer");
                return;
            }
        ...
        public override void OnStartClient()
        {
            if (roomPlayerPrefab == null || roomPlayerPrefab.gameObject == null)
                logger.LogError("NetworkRoomManagerExt no RoomPlayer prefab is registered. Please add a RoomPlayer prefab.");
            else
                ClientScene.RegisterPrefab(roomPlayerPrefab.gameObject);

            if (playerPrefab == null)
                logger.LogError("NetworkRoomManagerExt no GamePlayer prefab is registered. Please add a GamePlayer prefab.");
