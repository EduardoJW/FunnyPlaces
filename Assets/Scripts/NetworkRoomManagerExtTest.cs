...
using Mirror;
    ...
    /// </summary>
    /// <remarks>
    /// <para>The room has slots that track the joined players, and a maximum player count that is enforced. It requires that the NetworkRoomPlayerExtTest component be on the room player objects.</para>
    /// <para>NetworkRoomManagerExtTest is derived from NetworkManager, and so it implements many of the virtual functions provided by the NetworkManager class. To avoid accidentally replacing functionality of the NetworkRoomManagerExtTest, there are new virtual functions on the NetworkRoomManagerExtTest that begin with "OnRoom". These should be used on classes derived from NetworkRoomManagerExtTest instead of the virtual functions on NetworkManager.</para>
    /// <para>The OnRoom*() functions have empty implementations on the NetworkRoomManagerExtTest base class, so the base class functions do not have to be called.</para>
    /// </remarks>
    [AddComponentMenu("Network/NetworkRoomManagerExtTest")]
    [HelpURL("https://mirror-networking.com/docs/Components/NetworkRoomManagerExtTest.html")]
    public class NetworkRoomManagerExtTest : NetworkManager
    {
        static readonly ILogger logger = LogFactory.GetLogger(typeof(NetworkRoomManagerExtTest));
        ...
        protected NetworkRoomPlayerExtTest roomPlayerPrefab;
        ...
        public List<NetworkRoomPlayerExtTest> roomSlots = new List<NetworkRoomPlayerExtTest>();
        ...
        internal void ReadyStatusChanged()
        {
            ...
            foreach (NetworkRoomPlayerExtTest item in roomSlots)
            {
                if (item != null)
                {
                    CurrentPlayers++;
        ...
        {
            logger.Log("NetworkRoomManagerExtTest OnServerReady");
            ...
            if (conn != null && conn.identity != null)
            {
                ...
                if (roomPlayer != null && roomPlayer.GetComponent<NetworkRoomPlayerExtTest>() != null)
        ...
        /// <para>If all of the players are ready, then the server switches from the RoomScene to the PlayScene, essentially starting the game. This is called automatically in response to NetworkRoomPlayerExtTest.CmdChangeReadyState.</para>
        /// </summary>
        public void CheckReadyToBegin()
        {
            ...
            int numberOfReadyPlayers = NetworkServer.connections.Count(conn => conn.Value != null && conn.Value.identity.gameObject.GetComponent<NetworkRoomPlayerExtTest>().readyToBegin);
        ...
        {
            OnRoomClientEnter();
            foreach (NetworkRoomPlayerExtTest player in roomSlots)
                if (player != null)
                {
        ...
        public override void OnServerDisconnect(NetworkConnection conn)
        {
            ...
            {
                NetworkRoomPlayerExtTest roomPlayer = conn.identity.GetComponent<NetworkRoomPlayerExtTest>();
                ...
                {
                    roomPlayer = clientOwnedObject.GetComponent<NetworkRoomPlayerExtTest>();
                    if (roomPlayer != null)
                        roomSlots.Remove(roomPlayer);
                }
            }
            ...
            {
                if (player != null)
                    player.GetComponent<NetworkRoomPlayerExtTest>().readyToBegin = false;
            }
        ...
        public override void OnServerAddPlayer(NetworkConnection conn)
        {
            ...
            if (IsSceneActive(RoomScene))
            {
                ...
                if (logger.LogEnabled()) logger.LogFormat(LogType.Log, "NetworkRoomManagerExtTest.OnServerAddPlayer playerPrefab:{0}", roomPlayerPrefab.name);
        ...
        public override void ServerChangeScene(string newSceneName)
        {
            ...
            {
                foreach (NetworkRoomPlayerExtTest roomPlayer in roomSlots)
                {
                    if (roomPlayer == null)
                        continue;
                    ...
                    if (NetworkServer.active)
                    {
                        ...
                        roomPlayer.GetComponent<NetworkRoomPlayerExtTest>().readyToBegin = false;
        ...
        public override void OnStartServer()
        {
            ...
            {
                logger.LogError("NetworkRoomManagerExtTest RoomScene is empty. Set the RoomScene in the inspector for the NetworkRoomMangaer");
                return;
            }

            if (string.IsNullOrEmpty(GameplayScene))
            {
                logger.LogError("NetworkRoomManagerExtTest PlayScene is empty. Set the PlayScene in the inspector for the NetworkRoomMangaer");
                return;
            }
        ...
        public override void OnStartClient()
        {
            if (roomPlayerPrefab == null || roomPlayerPrefab.gameObject == null)
                logger.LogError("NetworkRoomManagerExtTest no RoomPlayer prefab is registered. Please add a RoomPlayer prefab.");
            else
                ClientScene.RegisterPrefab(roomPlayerPrefab.gameObject);

            if (playerPrefab == null)
                logger.LogError("NetworkRoomManagerExtTest no GamePlayer prefab is registered. Please add a GamePlayer prefab.");
